...
class BertTokenizer(EncoderInterface):
    ...
    """
    Parameter
    coulmns: defines the coulumns which are converted to canonicalized and tokenized rxnsmiles (only one column can be handled)
    ...
    def convert(self, dataset:Dataset, columns:List[str], n_jobs:int, **kwargs):
        self.__convertRXNSMILEStoTOKENS(InputBuffer=dataset.getFeature_data(), columns=columns, n_jobs=n_jobs)


    def __convertRXNSMILEStoTOKENS(self, InputBuffer:BatchTable, columns:List[str], n_jobs: int):
    ...
    def _parallel_convert(self, input_arr: Shared_PythonList, columns: List[str], out_dtypes, current_chunk:int):
        tokenizer = SmilesTokenizer(pkg_resources.resource_filename("rxnfp", "models/transformers/bert_pretrained/vocab.txt"))
     ...
     #                       tokenized_rxn = ' '.join(tokenizer.add_special_tokens_single_sequence(tokenizer.tokenize(canonical_rxn)[:tokenizer.max_len_single_sentence]))
                            tokenized_rxn = ' '.join(tokenizer.tokenize(canonical_rxn))                #Test??? Difference???

                        except Exception as e:
                            print(e)

                        if tokenized_rxn is None:
                            ...
                            out_array[index_counter][exists_col] = tokenized_rxn
                else:
