...
class rxnfp_BERT(Abstract_Model):
    clf = None
    ...
    def __init__ (self, param, log_folder:str, APP_ENV:Application_env):
        ...
        if device:
            self.clf = SmilesClassificationModel('bert', bert_path, num_labels=1, cuda_device=device, args=model_args)
        else:
            self.clf = SmilesClassificationModel('bert', bert_path, num_labels=1, args=model_args)
    ...
    def predict(self, X):
        X = np.ravel(X)
        predictions, raw_output = self.clf.predict(X)
        return predictions

    @staticmethod
    def getItemname():
        return "rxnfp_BERT_regressor"
